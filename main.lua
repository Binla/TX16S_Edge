...
local function refresh(_j7, event, touchState)
    ...
    if _n4 then
        if _k3[14][2] then
            ...
            lcd.drawText(312, 23, "Armed", CENTER + VCENTER +  RED)
        end
    else
        if _k3[13][2] then
            lcd.drawText(312, 23, "DisArmed", CENTER + VCENTER +   GREEN)
        elseif _j7.options.ArmSwitch ~= 0 then
            lcd.drawText(312, 23, "DisArmed", CENTER + VCENTER +   GREEN)
        else
            lcd.drawText(312, 23, "NO TELE", CENTER + VCENTER +   BLINK + RED)
        end
    end
    ...
    local _lbl_col = lcd.RGB(200, 100, 255)
    ...
    local _t1_obj = model.getTimer(0)
    local _t1_val = _t1_obj and _t1_obj.value or 0
    local _t1_abs = math.abs(_t1_val)
    local _t1_min = math.floor(_t1_abs / 60)
    local _t1_sec = _t1_abs % 60
    local _t1_sign = _t1_val < 0 and "-" or ""
    local _t3 = string.format("%s%02d:%02d", _t1_sign, _t1_min, _t1_sec)
    local _orig_time = string.format("%s:%s", _y4[1], _z5[1])
    
    local _t1_color = _t1_val < 0 and RED or _e7
